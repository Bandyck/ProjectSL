...
USLFadeComponent::USLFadeComponent()
{
...
void USLFadeComponent::BeginPlay()
{
	Super::BeginPlay();
	...
	if (bEnable)
	{
		GetWorld()->GetTimerManager().SetTimer(timerHandle_AddObjectsTimer, this, &USLFadeComponent::AddObjectsToHide,
			addObjectInterval, true);
		GetWorld()->GetTimerManager().SetTimer(timerHandle_ObjectComputeTimer, this, &USLFadeComponent::FadeObjWorker,
...
// Called every frame
void USLFadeComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void USLFadeComponent::FadeObjWorker()
{
...
void USLFadeComponent::SetActivate()
{
	bEnable = true;
}

void USLFadeComponent::SetDeactivate()
{
	bEnable = false;
}
